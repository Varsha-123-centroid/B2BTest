...
import AirlineListReturn from './AirlineListReturn';
  ...
  const openModalMessage = () => { 
    ...
    useEffect(() => {
      if (responseget && responseget.Response && responseget.Response.Results) {
        ...
        if (responseget.Response.Results[0]) {
          responseget.Response.Results[0].forEach((item) => {
...
const openModalAlert = () => { 
  ...
  //  const [result,setResult]=useState(response);
    ...
    const handleButtonClick = async (vl,trid,lcc,price11) => {
    const price12= parseFloat(price11) + parseFloat(price11) * parseFloat(markuppercent) + parseFloat(markup);
	
    if(parseFloat(price12)<parseFloat(balance))
        {  
          openModalAlert();
          await wait(3000);
 ...
 <div data-testid="u_policy_wrapper_2-0" className="policy-wrapper is-v2" style={{ display: isVisible ? "block" : "none" }}>
  <div className="policy-wrapper_content-wrapper">
    {selectedRow.resu?.Options ? (
      ...
      <button onClick={() => closeModalAlert()} style={{color:"white",padding:"7px",fontSize:"20px",backgroundColor:"red",marginLeft:"-25px"}}><i className="fas fa-times"></i></button> 
      </div>
      </div>
		<h2 className="text-3xl font-bold mb-6 text-center text-white">
		  <span className="bg-gradient-to-r text-transparent from-blue-500 to-purple-500 bg-clip-text">
			<h4  style={{fontSize:"15px",color:"grey"}}>Return Trip</h4>
		  </span>
		  <img src="assets/images/airimg.jpg" alt="" style={{width:"300px",height:"auto"}}/>
		  <h5 style={{fontSize:"15px",color:"grey"}}>{msgrtn}</h5>
...
export default AirlineListRound
